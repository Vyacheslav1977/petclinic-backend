...
async function parsePetsRequest({body, id, res, req}) {
  ...
  if (req.method === 'OPTIONS') {
    ...
    let rows = await petsRepository.search(JSON.parse(data).name);
    sendJSON({res, code: 200, data: rows});
  }
  else if (req.method === 'POST') { // Create
    let data = await getDataFromRequest(req);
    let row = await petsRepository.create(JSON.parse(data).name);
    ...
    await petsRepository.update({id, ...JSON.parse(data)});
    if (clientId !== null) {
